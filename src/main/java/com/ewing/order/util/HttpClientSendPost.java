...
public class HttpClientSendPost {
	...
	public static String sendXMLDataByPost(String url, String xmlData) throws ClientProtocolException, IOException {
		...
		// post.getParams().setParameter(HTTP.DEFAULT_PROTOCOL_CHARSET,
		// HTTP.UTF_8);

		// Construct a string entity
		StringEntity entity = new StringEntity(getUTF8XMLString(xmlData), Charsets.UTF_8.name());
		...
		String strrep = "";
		if (entityRep != null) {
			strrep = IOUtils.toString(response.getEntity().getContent());
			// Do not need the rest
			post.abort();
		}
