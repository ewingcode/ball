...
public class CalData2 {
	private static Logger log = LoggerFactory.getLogger(CalData2.class);
	@Resource
	...
	public void testOneGame() {
		String gid = "2594650";
		BetInfo entity = baseDao.findOne("select * from bet_info where gid='" + gid + "'",
				BetInfo.class);
		List<BetInfo> betInfoList = Lists.newArrayList();
		List<BetInfoDto> betInfoDtoList = Lists.newArrayList();
		betInfoList.add(entity);
		betInfoDtoList = betRollInfoService.fillMaxMinInfo(betInfoList);
		for (BetInfoDto betInfo : betInfoDtoList) {
			printGame(betInfo);
		}
		List<BetRollInfo> list = betRollInfoDao.find(gid);
